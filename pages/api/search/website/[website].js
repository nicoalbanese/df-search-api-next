...
export default async function handler(req, res) {
  ...
  if (api == process.env.NEXT_PUBLIC_APP_API_KEY) {
    ...
    let resultObj = {}
    if (results.length > 0) {
      resultObj = { query: website, count: results.length, results }
    } else {
      resultObj = {query: website, message: `${website} does not exist in the pipeline, follow the link below to create it.`, url: `https://airtable.com/shrUB5NNy0PGzPjQT?prefill_Website=https://${encodeURI(website)}`}
    }
...
const searchPipeline = async (domain) => {
  ...
  const AIRTABLE_URL =
    `https://api.airtable.com/v0/apptcOM65nkIWJy1l/Pipeline?filterByFormula=SEARCH("${domain}", lower({Website}))`;
